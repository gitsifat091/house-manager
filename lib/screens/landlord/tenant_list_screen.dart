...
class _TenantListScreenState extends State<TenantListScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: StreamBuilder<List<TenantModel>>(
          ...
          builder: (context, snap) {
            ...
            final raw = (snap.data ?? []).where((t) => t.isActive).toList();
            final tenants = _sortTenants(raw);
            final totalRent = tenants.fold<double>(0, (sum, t) => sum + t.rentAmount);
                      ...
                      delegate: SliverChildBuilderDelegate(
                        (ctx, i) => _TenantCard(
                          ...
                          onEdit: () => Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (_) =>
                                  LandlordEditTenantScreen(tenant: tenants[i]),
                            ),
                          ),
                        ),
                        childCount: tenants.length,
      ...
      floatingActionButton: Builder(builder: (ctx) {
        final u = ctx.read<AuthService>().currentUser!;
        return Container(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(16),
            boxShadow: [
          ...
          child: FloatingActionButton.extended(
            onPressed: () => Navigator.push(ctx,
                MaterialPageRoute(builder: (_) => AddEditTenantScreen(landlordId: u.uid))),
            icon: const Icon(Icons.person_add_rounded),
            label: const Text('ভাড়াটিয়া যোগ করুন',
                style: TextStyle(fontWeight: FontWeight.w700)),
          ...
          child: Row(
            children: [
              ...
              _statPill(
                icon: Icons.home_rounded,
                label: 'Properties',
                value: tenants.map((t) => t.propertyName).toSet().length.toString(),
                color: const Color(0xFFD97706),
                isDark: isDark,
              ),
            ],
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(Icons.people_outline_rounded, size: 46, color: primary.withOpacity(0.5)),
          ),
          const SizedBox(height: 20),
          const Text('কোনো ভাড়াটিয়া নেই',
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
...
class _TenantCard extends StatelessWidget {
            ...
            child: Column(
              children: [
                      ...
                      onSelected: (val) async {
                        ...
                        if (val == 'remove') {
                          final confirm = await _confirm(
                            context,
                            'রুম খালি করবেন?',
                            '${tenant.name} কে রুম থেকে সরানো হবে এবং archive এ যাবে।',
                          );
                          if (confirm) {
                            // ✅ moveOutDate সহ archive করা হচ্ছে
                            await _removeTenantWithMoveOutDate(context);
                          }
                        }
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      _infoChip(Icons.home_outlined, tenant.propertyName, primary),
                      const SizedBox(width: 8),
                      _infoChip(Icons.door_front_door_outlined, 'রুম ${tenant.roomNumber}', primary),
                      const Spacer(),
                      Row(
                        children: [
                          Icon(Icons.calendar_today_outlined,
                              size: 11, color: textSecondary.withOpacity(0.7)),
                          const SizedBox(width: 4),
                          Text(
                            '${tenant.moveInDate.day}/${tenant.moveInDate.month}/${tenant.moveInDate.year}',
  ...
  // ── রুম খালি করা + moveOutDate Firestore এ save ──────────────────────────
  // TenantService.removeTenant() কে bypass করে সরাসরি Firestore update করছি
  // কারণ moveOutDate save করতে হবে এবং isActive: false করতে হবে একসাথে।
  Future<void> _removeTenantWithMoveOutDate(BuildContext context) async {
    final db = FirebaseFirestore.instance;
    final now = Timestamp.now();

    try {
      // ১. Tenant কে archive করো — isActive: false + moveOutDate save
      await db.collection('tenants').doc(tenant.id).update({
        'isActive': false,
        'moveOutDate': now, // ✅ ছেড়ে যাওয়ার তারিখ
      });

      // ২. Room কে vacant করো
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('${tenant.name} কে archive করা হয়েছে'),
  ...
  Widget _infoChip(IconData icon, String label, Color primary) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        Icon(icon, size: 12, color: primary),
  ...
  Future<bool> _confirm(BuildContext context, String title, String content) async {
    return await showDialog<bool>(
          ...
          builder: (_) => AlertDialog(
            ...
            actions: [
              TextButton(
                  onPressed: () => Navigator.pop(context, false), child: const Text('না')),
              FilledButton(
                  onPressed: () => Navigator.pop(context, true),
                  child: const Text('হ্যাঁ')),
            ],
