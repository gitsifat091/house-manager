...
const Map<String, List<String>> _divisionDistricts = {
  'ঢাকা': [
    ...
    'নরসিংদী', 'রাজবাড়ী', 'মাদারীপুর', 'গোপালগঞ্জ', 'ফরিদপুর',
    'শরিয়তপুর', 'কিশোরগঞ্জ', 'টাঙ্গাইল',
  ],
  'চট্টগ্রাম': [
    'চট্টগ্রাম', 'কক্সবাজার', 'ফেনী', 'নোয়াখালী', 'লক্ষ্মীপুর',
    'কুমিল্লা', 'চাঁদপুর', 'ব্রাহ্মণবাড়িয়া', 'খাগড়াছড়ি', 'রাঙামাটি', 'বান্দরবান',
  ],
  'রাজশাহী': [
    'রাজশাহী', 'নাটোর', 'নওগাঁ', 'চাঁপাইনবাবগঞ্জ', 'পাবনা',
...
class _FindHomeScreenState extends State<FindHomeScreen> {
  ...
  final List<String> _divisions = ['সব', ..._divisionDistricts.keys];
  final List<String> _roomTypes = ['সব', 'Single', 'Double', 'Family', 'Bachelor'];

  List<String> get _currentDistricts =>
      (_selectedDivision == 'সব') ? [] : (_divisionDistricts[_selectedDivision] ?? []);
  ...
  Future<void> _search() async {
    ...
    try {
      final results = await _service.searchListings(
        division: _selectedDivision == 'সব' ? null : _selectedDivision,
  ...
  Widget build(BuildContext context) {
    ...
    final textSecondary = isDark ? Colors.white54 : const Color(0xFF6B7280);
    final cardBg = isDark ? const Color(0xFF1A2C22) : Colors.white;
    final fillColor = isDark ? const Color(0xFF0F1A14) : const Color(0xFFF5FAF7);
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          onChanged: (v) => setState(() {
                            _selectedDivision = v ?? 'সব';
                            _selectedDistrict = null;
                          }),
                        ),
                        const SizedBox(height: 10),

                        // ── জেলা (full width, আলাদা row) ──
                        DropdownButtonFormField<String>(
                          key: ValueKey(_selectedDivision),
                          value: _selectedDistrict,
                          isExpanded: true,           // ← এটা জরুরি
                          decoration: InputDecoration(
                            labelText: 'জেলা',
                            prefixIcon: const Icon(Icons.location_city_outlined),
                            filled: true,
                          ...
                          items: [
                            const DropdownMenuItem(value: null, child: Text('সব জেলা')),
                            ..._currentDistricts
                                .map((d) => DropdownMenuItem(value: d, child: Text(d))),
                          ],
                          onChanged: _currentDistricts.isEmpty
                              ? null
                              : (v) => setState(() => _selectedDistrict = v),
                        ),
                        const SizedBox(height: 10),

                        // ── থানা (full width, আলাদা row) ──
                        TextField(
                          controller: _thanaTF,
                          decoration: InputDecoration(
                            labelText: 'থানা/এলাকা',
                        ...
                        // ── সর্বোচ্চ ভাড়া + ধরন (এই Row ঠিকই আছে) ──
                        Row(
                          children: [
                            Expanded(
                              child: TextField(
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _chip('${listing.bathrooms} বাথ', Icons.bathroom_outlined, const Color(0xFF059669)),
                    if (listing.floorSize != null)
                      _chip('${listing.floorSize!.toStringAsFixed(0)} sqft',
                          Icons.square_foot_outlined, const Color(0xFFD97706)),
                    _chip(listing.roomType, Icons.people_outline_rounded, primary),
                    if (listing.hasBalcony)
                      _chip('বারান্দা', Icons.balcony_outlined, const Color(0xFF5B4FBF)),
                    if (listing.hasParking)
                      _chip('পার্কিং', Icons.local_parking_outlined, const Color(0xFF6B7280)),
                  ],
                ),
...
class _ListingDetailScreenState extends State<ListingDetailScreen> {
  ...
  Future<void> _checkAlreadyRequested() async {
    ...
    final snap = await _service.getTenantRequests(user.uid).first;
    setState(() {
      _alreadyRequested = snap.any((r) =>
          r.roomId == widget.listing.roomId &&
          r.status == RentalRequestStatus.pending);
    });
  }
  ...
  Future<void> _sendRequest() async {
    if (_nameCtrl.text.trim().isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('নাম, ফোন ও NID দিন'), backgroundColor: Colors.red));
      return;
    }
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              '${listing.address}, ${listing.thana}, ${listing.district}',
                              style: const TextStyle(color: Colors.white70, fontSize: 13),
                            ),
                          ),
                        ],
                ...
                // Details grid
                _card(cardBg, [_detailGrid(listing, textPrimary, textSecondary)]),
                const SizedBox(height: 12),
                ...
                Wrap(
                  ...
                  children: [
                    if (listing.hasDiningRoom)
                      _amenity(context, 'ডাইনিং রুম', Icons.dining_outlined, primary),
                    if (listing.hasBalcony)
                      _amenity(context, 'বারান্দা', Icons.balcony_outlined, const Color(0xFF0891B2)),
                    if (listing.hasParking)
                      _amenity(context, 'পার্কিং', Icons.local_parking_outlined, const Color(0xFF059669)),
                    if (listing.hasLift)
                      _amenity(context, 'লিফট', Icons.elevator_outlined, const Color(0xFF5B4FBF)),
                    if (listing.hasGenerator)
                      _amenity(context, 'জেনারেটর', Icons.bolt_outlined, const Color(0xFFD97706)),
                    if (listing.isGasPiped)
                      _amenity(context, 'পাইপ গ্যাস', Icons.local_fire_department_outlined, Colors.red),
                  ],
                ),
                const SizedBox(height: 14),
                ...
                Wrap(
                  ...
                  children: [
                    if (listing.allowsFamily)
                      _amenity(context, 'পরিবার', Icons.family_restroom_outlined, Colors.green),
                    if (listing.allowsBachelor)
                      _amenity(context, 'ব্যাচেলর', Icons.person_outlined, const Color(0xFF0891B2)),
                    if (listing.allowsStudent)
                      _amenity(context, 'ছাত্র/ছাত্রী', Icons.school_outlined, const Color(0xFFD97706)),
                  ],
                ),
                ...
                if (_alreadyRequested)
                  Container(
                    ...
                    child: const Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'আপনি ইতিমধ্যে এই রুমের জন্য আবেদন করেছেন',
                            style: TextStyle(
                  ...
                  _card(cardBg, [
                    Padding(
                      ...
                      child: Column(
                        children: [
                          _field(context, _nameCtrl, 'পুরো নাম', Icons.person_outline_rounded),
                          const SizedBox(height: 10),
                          _field(context, _phoneCtrl, 'ফোন নম্বর', Icons.phone_outlined,
                              type: TextInputType.phone),
                          const SizedBox(height: 10),
                          _field(context, _emailCtrl, 'Email', Icons.email_outlined,
                              type: TextInputType.emailAddress),
                          const SizedBox(height: 10),
                          _field(context, _nidCtrl, 'NID নম্বর', Icons.badge_outlined,
                              type: TextInputType.number),
                          const SizedBox(height: 10),
                          _field(context, _msgCtrl,
                              'বাড়ীওয়ালার জন্য বার্তা (ঐচ্ছিক)',
    ...
    return Padding(
      ...
      child: GridView.count(
        ...
        children: [
          ...
          _gridItem('${l.floor}তলা', 'তলা', Icons.stairs_outlined, const Color(0xFF5B4FBF)),
          if (l.floorSize != null)
            _gridItem('${l.floorSize!.toStringAsFixed(0)}', 'বর্গফুট',
                Icons.square_foot_outlined, Colors.teal),
          _gridItem(l.roomType, 'ধরন', Icons.people_outline_rounded, const Color(0xFF059669)),
        ],
      ),
    );
  ...
  Widget _amenity(BuildContext context, String label, IconData icon, Color color) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
      decoration: BoxDecoration(
